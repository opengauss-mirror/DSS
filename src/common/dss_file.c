...
static status_t dss_refresh_dir_r(
    dss_session_t *session, dss_vg_info_item_t *vg_item, gft_node_t *parent_node, bool8 is_refresh_all_node)
{
    ...
    for (uint32 i = 0; i < parent_node->items.count; i++) {
        ...
        gft_node_t *node = dss_get_ft_node_by_ftid(session, vg_item, id, CM_TRUE, CM_TRUE);
        if (node == NULL) {
        ...
        if (node->type == GFT_PATH && is_refresh_all_node) {
            status = dss_refresh_dir_r(session, vg_item, node, is_refresh_all_node);
            if (status != CM_SUCCESS) {
...
status_t dss_open_dir(dss_session_t *session, const char *dir_path, bool32 is_refresh, dss_find_node_t *find_info)
{
    ...
    do {
        ...
        if (is_refresh) {
            ...
            cm_panic(node != NULL);
            status = dss_refresh_dir_r(session, vg_item, node, CM_TRUE);
            if (status != CM_SUCCESS) {
    ...
    dss_session_t *session, const char *path, uint32 type, gft_node_t **out_node, dss_find_node_t *find_info)
{
    ...
    DSS_RETURN_IFERR3(status, dss_rollback_mem_update(session, vg_item), dss_unlock_vg_mem_and_shm(session, vg_item));

    find_info->ftid = (*out_node)->id;
    errno = strncpy_sp(find_info->vg_name, DSS_MAX_NAME_LEN, vg_item->vg_name, DSS_MAX_NAME_LEN - 1);
    bool32 result = (bool32)(errno == EOK);
    DSS_RETURN_IF_FALSE3(
        result, dss_rollback_mem_update(session, vg_item), dss_unlock_vg_mem_and_shm(session, vg_item));
...
status_t dss_init_ft_block(
    dss_session_t *session, dss_vg_info_item_t *vg_item, char *block, uint32_t block_id, auid_t auid)
{
    ...
    if (ft_block->node_num > 0) {
        ...
        if (!cmp) {
            last_node = dss_get_ft_node_by_ftid(session, vg_item, gft->free_list.last, CM_FALSE, CM_FALSE);
            if (last_node == NULL) {
                LOG_DEBUG_ERR(
...
status_t dss_format_ft_node_core(
    dss_session_t *session, dss_vg_info_item_t *vg_item, ga_queue_t queue, auid_t auid, gft_root_t *gft)
{
    status_t status = CM_SUCCESS;
    ...
    for (uint32 i = 0; i < block_num; i++) {
        ...
        do {
            ...
            status = dss_init_ft_block(session, vg_item, (char *)block, i, auid);
            if (status != CM_SUCCESS) {
                rollback_count = i + 1;
                DSS_BREAK_IFERR2(status,
...
status_t dss_format_ft_node(dss_session_t *session, dss_vg_info_item_t *vg_item, auid_t auid)
{
    ...
    status = dss_format_ft_node_core(session, vg_item, queue, auid, gft);
    if (status != CM_SUCCESS) {
        return status;
    }
...
static void dss_get_prev_and_next_node(dss_session_t *session, dss_vg_info_item_t *vg_item, gft_node_t *parent_node,
    gft_node_t *node, gft_block_info_t *prev_info, gft_block_info_t *next_info)
{
    ...
    if (*(uint64 *)(&parent_node->items.first) == *(uint64 *)(&node->id)) {
        ...
        if (cmp) {
            ...
            next_info->ft_node = dss_get_ft_node_by_ftid(session, vg_item, parent_node->items.first, CM_TRUE, CM_FALSE);
        ...
        prev_info->ft_node = dss_get_ft_node_by_ftid(session, vg_item, parent_node->items.last, CM_TRUE, CM_FALSE);
        CM_ASSERT(prev_info->ft_node != NULL);
        dss_set_blockid(&prev_info->ft_node->next, CM_INVALID_ID64);
    } else {
        prev_info->ft_node = dss_get_ft_node_by_ftid(session, vg_item, node->prev, CM_TRUE, CM_FALSE);
        CM_ASSERT(prev_info->ft_node != NULL);
        prev_info->ft_node->next = node->next;
        next_info->ft_node = dss_get_ft_node_by_ftid(session, vg_item, node->next, CM_TRUE, CM_FALSE);
        CM_ASSERT(next_info->ft_node != NULL);
...
status_t dss_refresh_root_ft_inner(dss_vg_info_item_t *vg_item)
{
    bool32 remote = CM_TRUE;
    dss_ctrl_t *dss_ctrl = vg_item->dss_ctrl;
    char *root = dss_ctrl->root;
    status_t status = dss_load_vg_ctrl_part(vg_item, (int64)DSS_CTRL_ROOT_OFFSET, root, (int32)DSS_BLOCK_SIZE, &remote);
...
status_t dss_refresh_root_ft(dss_vg_info_item_t *vg_item, bool32 check_version, bool32 active_refresh)
{
    ...
    if (check_version) {
        ...
        if (dss_compare_version(disk_version, version)) {
            ...
            status = dss_refresh_root_ft_inner(vg_item);
            DSS_RETURN_IF_ERROR(status);
        }
    }
    return CM_SUCCESS;
    ...
    dss_session_t *session, dss_vg_info_item_t *vg_item, ftid_t id)
{
    ...
    if (is_ft_root_block(id)) {
        char *root = dss_ctrl->root;
        dss_root_ft_block_t *ft_block = (dss_root_ft_block_t *)(root);
        if (dss_refresh_root_ft_inner(vg_item) != CM_SUCCESS) {
            return NULL;
        }
        if (id.item < ft_block->ft_block.node_num) {
            return (gft_node_t *)((root + sizeof(dss_root_ft_block_t)) + id.item * sizeof(gft_node_t));
        ...
        dss_block_id_t block_id = id;
        block_id.item = 0;
        dss_ft_block_t *ft_block = (dss_ft_block_t *)dss_find_block_from_disk_and_refresh_shm(
            session, vg_item, block_id, DSS_BLOCK_TYPE_FT, NULL);
        if (ft_block == NULL) {
            LOG_DEBUG_ERR("Failed to find block:%s from disk and refresh shm.", dss_display_metaid(block_id));
            return NULL;
        }
        if (ft_block->node_num <= id.item) {
...
char *dss_get_ft_block_by_ftid(dss_session_t *session, dss_vg_info_item_t *vg_item, ftid_t id)
{
    ...
    return dss_find_block_in_shm(session, vg_item, id, DSS_BLOCK_TYPE_FT, CM_TRUE, NULL, CM_FALSE);
}
...
status_t dss_update_ft_root(dss_vg_info_item_t *vg_item)
{
    ...
    DSS_LOG_DEBUG_OP("Update node table root, version:%llu, checksum:%u.", block->ft_block.common.version,
    ...
    dss_vg_info_item_t *vg_item, dss_block_id_t blockid, char *block, bool32 *is_changed)
{
    ...
    return dss_check_block_version(vg_item, blockid, DSS_BLOCK_TYPE_FS, block, is_changed, CM_FALSE);
}
...
status_t dss_refresh_ft(dss_session_t *session, dss_vg_info_item_t *vg_item)
{
    ...
    while (!cmp) {
        ft_block = (dss_root_ft_block_t *)dss_get_ft_block_by_ftid(session, vg_item, block_id);
        if (ft_block) {
...
static status_t dss_get_second_block(
    dss_session_t *session, dss_vg_info_item_t *vg_item, dss_block_id_t second_block_id, dss_fs_block_t **second_block)
{
    *second_block = (dss_fs_block_t *)dss_find_block_in_shm(
        session, vg_item, second_block_id, DSS_BLOCK_TYPE_FS, CM_TRUE, NULL, CM_FALSE);
    if (!*second_block) {
        DSS_RETURN_IFERR2(
...
status_t dss_alloc_fs_aux_batch_prepare(dss_session_t *session, dss_vg_info_item_t *vg_item, uint32 batch_count)
{
    CM_ASSERT(vg_item != NULL);
    status_t status = CM_SUCCESS;
    auid_t auid;
    dss_fs_aux_root_t *root = DSS_GET_FS_AUX_ROOT(vg_item->dss_ctrl);
    if (root->free.count >= batch_count) {
        LOG_DEBUG_INF("[FS AUX]No need to alloc new au, root free count is %llu, expect count is %u", root->free.count,
            batch_count);
        return CM_SUCCESS;
    }
    while (root->free.count < batch_count) {
        status = dss_alloc_au(session, vg_item, &auid);
    ...
    LOG_DEBUG_INF(
        "[FS AUX]Succeed to alloc new au, root free count is %llu, expect count is %u", root->free.count, batch_count);
    return CM_SUCCESS;
}

status_t dss_extend_fs_aux_batch_inner(dss_session_t *session, dss_vg_info_item_t *vg_item, auid_t batch_first,
    gft_node_t *node, uint32 block_au_count, uint32 batch_count, dss_fs_block_t *second_block,
    uint64 old_aux_root_free_count)
{
    dss_fs_aux_t *fs_aux;
    dss_block_id_t block_id;
    dss_fs_aux_root_t *root = DSS_GET_FS_AUX_ROOT(vg_item->dss_ctrl);
    dss_redo_set_fs_aux_block_batch_t redo_batch;
    redo_batch.fs_block_id = second_block->head.common.id;
    redo_batch.batch_count = (uint16)batch_count;
    redo_batch.old_used_num = second_block->head.used_num;
    redo_batch.node_id = node->id;
    redo_batch.first_batch_au = batch_first;
    auid_t auid;
    uint16 index;
    for (uint32 i = 0, j = block_au_count; i < batch_count; i++, j++) {
        ...
        index = (uint16)j;
        block_id = root->free.first;
        if (root->free.count <= old_aux_root_free_count) {
        ...
        LOG_DEBUG_INF("[FS AUX]Init fs aux, fs aux id:%s, data_id:%s.", dss_display_metaid(fs_aux->head.common.id),
        ...
        redo_batch.id_set[i] = second_block->bitmap[j];
        LOG_DEBUG_INF("[FS AUX]second block %s, bitmap %u, fs_aux %s, au %s.",
    ...
    redo_batch.new_free_list = root->free;
    dss_put_log(
        session, vg_item, DSS_RT_SET_FS_AUX_BLOCK_BATCH, &redo_batch, sizeof(dss_redo_set_fs_aux_block_batch_t));
    return CM_SUCCESS;
}

status_t dss_extend_fs_aux_batch(dss_session_t *session, dss_vg_info_item_t *vg_item, auid_t batch_first,
    gft_node_t *node, uint32 block_au_count, uint32 batch_count, dss_fs_block_t *second_block)
{
    dss_fs_aux_root_t *root = DSS_GET_FS_AUX_ROOT(vg_item->dss_ctrl);
    uint64 old_aux_root_free_count = root->free.count;
    status_t status = dss_alloc_fs_aux_batch_prepare(session, vg_item, batch_count);
    if (status != CM_SUCCESS) {
        char *err_msg = "Failed to allocate file space aux meta block, vg name";
        DSS_RETURN_IFERR2(CM_ERROR, LOG_DEBUG_ERR("%s %s.", err_msg, vg_item->dss_ctrl->vg_info.vg_name));
    }
    CM_ASSERT(root->free.count >= batch_count);
    CM_ASSERT(!dss_cmp_blockid(root->free.first, CM_INVALID_ID64));
    return dss_extend_fs_aux_batch_inner(
        session, vg_item, batch_first, node, block_au_count, batch_count, second_block, old_aux_root_free_count);
}
...
status_t dss_extend_fs_batch(dss_session_t *session, dss_vg_info_item_t *vg_item, auid_t batch_first,
    uint32 block_au_count, uint32 batch_count, dss_fs_block_t *second_block)
{
    ...
    uint64 align_end, bool32 *finish)
{
    *finish = CM_FALSE;
    ...
    if (!DSS_IS_FILE_INNER_INITED(node->flags)) {
        LOG_DEBUG_INF("normal file:%s fid:%llu not bitmap type, skip extend fs aux", node->name, node->fid);
        status = dss_extend_fs_batch(session, vg_item, batch_first, block_au_count, batch_count, second_block);
        if (status != CM_SUCCESS) {
            dss_rollback_mem_update(session, vg_item);
            char *err_msg = "Failed to alloc file meta block, vg name";
            DSS_RETURN_IFERR2(CM_ERROR, LOG_DEBUG_ERR("%s :%s.", err_msg, vg_item->dss_ctrl->vg_info.vg_name));
        }
    } else {
        status =
            dss_extend_fs_aux_batch(session, vg_item, batch_first, node, block_au_count, batch_count, second_block);
        if (status != CM_SUCCESS) {
            dss_rollback_mem_update(session, vg_item);
            char *err_msg = "Failed to alloc file space aux meta block, vg name";
            DSS_RETURN_IFERR2(CM_ERROR, LOG_DEBUG_ERR("%s :%s.", err_msg, vg_item->dss_ctrl->vg_info.vg_name));
...
static void dss_init_truncate_fs_block_batch_redo(dss_fs_block_t *src_fsb, dss_fs_block_t *dst_fsb, uint16 src_idx,
    uint16 dst_idx, dss_redo_truncate_fs_block_batch_t *redo)
{
    redo->src_id = src_fsb->head.common.id;
    redo->dst_id = dst_fsb->head.common.id;
    DSS_ASSERT_LOG(!dss_cmp_blockid(redo->src_id, DSS_INVALID_64), "src id should not be invalid id.");
    DSS_ASSERT_LOG(!dss_cmp_blockid(redo->dst_id, DSS_INVALID_64), "dst id should not be invalid id.");
...
static void dss_transfer_remaining_au_inner(dss_session_t *session, dss_vg_info_item_t *vg_item,
    dss_fs_block_t *src_partial_sfsb, dss_fs_block_t *dst_sfsb, uint32 *src_au_idx, uint32 *dst_au_idx,
    dss_redo_truncate_fs_block_batch_t *redo, auid_t *auid)
{
    uint16 count = 0;
    ...
    redo->count = count;
    if (count != 0) {
        dss_put_log(session, vg_item, DSS_RT_TRUNCATE_FS_BLOCK_BATCH, redo, sizeof(dss_redo_truncate_fs_block_batch_t));
    }
}
    ...
    dss_vg_info_item_t *vg_item)
{
    ...
    LOG_DEBUG_INF(
        "Begin to transfer the partial FSB, src_partial_sfsb: %s, src_au_idx:%u, %u AUs to xfer, src_used_num: %hu,"
    ...
    dss_fs_block_t *src_entry_fsb, dss_fs_block_t *dst_entry_fsb, uint32 curr_src_idx, int32 *dst_sfsb_idx)
{
    ...
    dss_redo_truncate_fs_block_batch_t redo;
    LOG_DEBUG_INF(
        "Begin to transfer the complete FSB, src_entry_fsb: %s, src_au_idx:%u, %u AUs to xfer, src_used_num: %hu,"
        "dst_entry_fsb: %s, dst_au_idx:%u, au_idx_limit:%u, dst_used num:%hu.",
    ...
    dss_init_truncate_fs_block_batch_redo(
        src_entry_fsb, dst_entry_fsb, (uint16)curr_src_idx, (uint16)(*dst_sfsb_idx), &redo);
    ...
    dss_fs_block_t *dst_entry_fsb, uint32 src_partial_sfsb_idx, uint32 src_au_idx)
{
    ...
    if (src_au_idx < au_idx_limit) {
        ...
        (void)dss_get_second_block(session, vg_item, src_partial_sfsb_id, &src_partial_sfsb);
        ...
        (void)dss_get_second_block(session, vg_item, dst_sfsb_id, &dst_sfsb);
        DSS_ASSERT_LOG(dst_sfsb != NULL, "Null pointer: dst_sfsb, dst_sfsb_id=%s.", dss_display_metaid(dst_sfsb_id));
        uint32 dst_au_idx = 0;
        ...
        (void)dss_get_second_block(session, vg_item, cache_first_sfsb, &dst_second_sfsb);
...
{
    char *block = dss_find_block_in_shm(session, vg_item, blockid, DSS_BLOCK_TYPE_FS, CM_FALSE, NULL, CM_FALSE);
    CM_ASSERT(block != NULL);
...
status_t dss_refresh_ft_block(dss_session_t *session, char *vg_name, uint32 vgid, dss_block_id_t blockid)
{
    ...
    if (node->type == GFT_PATH) {
        status = dss_refresh_dir_r(session, vg_item, node, CM_FALSE);
        DSS_RETURN_IFERR3(status,
    ...
    dss_session_t *session, uint32 vg_id, int64 offset, int64 size, dss_block_id_t ftid, uint64 fid)
{
    ...
    gft_node_t *node = NULL;
    // this function may be called by standby forwarded request (DSS_CMD_UPDATE_WRITTEN_SIZE, exec_on_active=true)
    // use force mode to avoid deadlock with IX lock during truncate broadcast
    if (session != NULL && session->is_remote_req) {
        dss_lock_vg_mem_and_shm_s_force(session, vg_item);
    } else {

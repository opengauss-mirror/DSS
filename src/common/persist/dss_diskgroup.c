...
status_t dss_alloc_vg_item_redo_log_buf(dss_vg_info_item_t *vg_item)
{
    ...
    if (log_buf == NULL) {
        DSS_RETURN_IFERR2(CM_ERROR, DSS_THROW_ERROR(ERR_ALLOC_MEMORY, DSS_VG_LOG_SPLIT_SIZE, "global log buffer"));
    }
...
status_t dss_load_vg_ctrl(dss_vg_info_item_t *vg_item, bool32 is_lock)
{
    ...
    if (vg_item->id == 0 && vg_item->dss_ctrl->vg_info.proto_version > DSS_PROTO_VERSION) {
        DSS_EXIT_LOG(DSS_FALSE, "Protocol version %u larger than software version %u.",
            vg_item->dss_ctrl->vg_info.proto_version, DSS_PROTO_VERSION);
    }
...
status_t dss_dl_dealloc(unsigned int lock_id)
{
...
status_t dss_lock_share_disk_vg(const char *entry_path, dss_config_t *inst_cfg)
{
    ...
    lock_id = cm_dl_alloc(entry_path, DSS_VG_LOCK_SHARE_DISK_OFFSET, (unsigned long long)inst_cfg->params.inst_id);
    ...
    for (;;) {
        ret = cm_dl_lock(lock_id, DSS_LOCK_VG_TIMEOUT_MS);
        if (ret == CM_SUCCESS) {
            LOG_DEBUG_INF("Lock vg succ, entry path %s.", entry_path);
            DSS_RETURN_IF_ERROR(dss_dl_dealloc(lock_id));
            return CM_SUCCESS;
        }
...
status_t dss_unlock_vg_storage(dss_vg_info_item_t *vg_item, const char *entry_path, dss_config_t *inst_cfg)
{
    if (dss_unlock_vg_storage_core(vg_item, entry_path, inst_cfg) != CM_SUCCESS) {
        dss_file_unlock_vg();
        return CM_ERROR;
    }
    dss_file_unlock_vg();
...
status_t dss_check_lock_remain_share_disk(
    dss_vg_info_item_t *vg_item, const char *entry_path, int64 inst_id, bool32 *is_remain)
{
    ...
    *is_remain = CM_TRUE;
    ...
    if (cm_dl_check_lock_remain(lock_id, (unsigned long long)inst_id, (unsigned int *)is_remain) != CM_SUCCESS) {
        (void)dss_dl_dealloc(lock_id);
        return CM_ERROR;
    }
    return dss_dl_dealloc(lock_id);
}

status_t dss_check_lock_remain_cluster_raid(
    dss_vg_info_item_t *vg_item, const char *entry_path, int64 inst_id, bool32 *is_remain)
{
    int32 fd = 0;
    dlock_t lock;
...
{
    if (dss_mode == DSS_MODE_SHARE_DISK) {
        return dss_check_lock_remain_share_disk(vg_item, entry_path, inst_id, is_remain);
    } else if (dss_mode == DSS_MODE_CLUSTER_RAID) {
        return dss_check_lock_remain_cluster_raid(vg_item, entry_path, inst_id, is_remain);
    }
    LOG_DEBUG_ERR("Invalid dss mode %d when check lock remain.", dss_mode);
    return CM_ERROR;
}
#endif
...
static status_t dss_refresh_core_ctrl(dss_vg_info_item_t *vg_item)
{
    ...
    LOG_RUN_INF("Try to load core ctrl, disk version:%llu, mem version:%llu for vg:%s.", disk_core_version,
        vg_item->dss_ctrl->core.version, vg_item->vg_name);
    ...
    LOG_RUN_INF("End to load core ctrl, disk version:%llu, mem version:%llu for vg:%s.", disk_core_version,
        vg_item->dss_ctrl->core.version, vg_item->vg_name);
    return CM_SUCCESS;
}
static status_t dss_refresh_volume_ctrl(dss_vg_info_item_t *vg_item)
{
    ...
    LOG_RUN_INF("Try to load volume ctrl, disk version:%llu, mem version:%llu for vg:%s.", disk_volume_version,
        vg_item->dss_ctrl->volume.version, vg_item->vg_name);
    ...
    LOG_RUN_INF("End to load volume ctrl, disk version:%llu, mem version:%llu for vg:%s.", disk_volume_version,
        vg_item->dss_ctrl->volume.version, vg_item->vg_name);
    return CM_SUCCESS;
}
...
status_t dss_load_ctrl_core(dss_vg_info_item_t *vg_item, uint32 index)
{
    ...
    switch (index) {
        ...
        case DSS_VG_INFO_REDO_CTRL:
            status = dss_load_vg_ctrl_part(vg_item, (int64)DSS_CTRL_REDO_OFFSET, vg_item->dss_ctrl->redo_ctrl_data,
...
status_t dss_load_ctrl(dss_session_t *session, const char *vg_name, uint32 index)
{
    ...
    if (status != CM_SUCCESS) {
        (void)dss_unlock_vg_storage(vg_item, vg_item->entry_path, inst_cfg);
        LOG_RUN_ERR("Failed to load vg data from disk, vg_name:%s, index %u.", vg_name, index);
        return status;
    }
    LOG_RUN_INF("Succeed to load ctrl data from disk, vg_name:%s, index %u.", vg_name, index);
    return dss_unlock_vg_storage(vg_item, vg_item->entry_path, inst_cfg);
}
...
status_t dss_refresh_meta_info(dss_session_t *session)
{
    ...
    for (uint32_t i = 0; i < g_vgs_info->group_num; i++) {
        ...
        status =
            dss_refresh_buffer_cache(session, &g_vgs_info->volume_group[i], g_vgs_info->volume_group[i].buffer_cache);
    ...
    dss_vg_info_item_t *vg_item, dss_volume_t *volume, int64 offset, void *buf, int32 size, bool32 *remote_chksum)
{
    ...
    while (get_instance_status_proc != NULL && get_instance_status_proc() != DSS_STATUS_RECOVERY &&
           dss_need_load_remote(size) == CM_TRUE && status != CM_SUCCESS) {

...
void dss_add_syn_meta(dss_vg_info_item_t *vg_item, dss_block_ctrl_t *block_ctrl, uint64 version)
{
    ...
    dss_buffer_recycle_disable(block_ctrl, CM_TRUE);
...
void dss_del_syn_meta(dss_vg_info_item_t *vg_item, dss_block_ctrl_t *block_ctrl, int64 syn_meta_ref_cnt)
{
    ...
    dss_buffer_recycle_disable(block_ctrl, CM_FALSE);
    dss_unlatch(&vg_item->syn_meta_desc.latch);
}
...
void dss_syn_meta(dss_session_t *session, dss_vg_info_item_t *vg_item, dss_block_ctrl_t *block_ctrl)
{
    if (dss_need_exec_local() && dss_is_readwrite()) {
        ...
        LOG_DEBUG_INF("syn meta file:%llu file_ver:%llu, vg:%u, block:%llu type:%u, with version:%llu.", req.data.fid,
            req.data.file_ver, req.data.vg_id, req.data.meta_block_id, req.data.meta_type, req.data.syn_meta_version);
    }
}
